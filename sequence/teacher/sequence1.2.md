...
sequenceDiagram
    ...
    TeacherView-->>-Teacher: 2.2. Chuyển đến trang sửa nội dung
    Teacher->>+TeacherView: 2.3. Sửa nội dung bài học
    Teacher->>+TeacherView: 2.4. Nhấn nút "Lưu"
    TeacherView->>+TeacherLessonController: 2.4.1. Gửi id bài học
    TeacherLessonController->>TeacherService: 2.4.2. UpdateLessonContent()
    alt ServiceValidate()
        TeacherService->>+TeacherRepository: 2.4.3. UpdateLessonContentByLessonIdAsync()
        TeacherRepository->>+Database: 2.4.4. executeQuery()

        Database->>-TeacherLessonController: 2.4.5. return result
        TeacherLessonController->>-TeacherView: 2.4.6.1. cập nhật nội dung buổi học thành công
    else invalid
